...
/* mNode 建立函數, 傳入 key ptr, value ptr, 建立新節點 */
            ...
            switch (func){
                case ADD: // 插入 -> 更新 value, 若未找到跳出迴圈 (tmp == NULL), return add() 新增 node
                    ...
                    if (value->type == V_STRING)
                        strcpy(tmp->value.data.stringVal, value->data.stringVal); // 複製字串
...
const char * getValue(Map * map, const int key){
    ...
    return (const char *) ret->data.stringVal;
}
