...
void count_conseq(node * start){
    ...
    print(start, tmp);
    puts(tmp);
    char result[BLOCK_LEN] = "", tmp_char;
    int cont_len = 0, tmp_len;
    _Bool take = 0;
    for (int i = 0; i < strlen(tmp); ++i){
        if (!take){ // init
            tmp_len = 1, tmp_char = tmp[i];
            take = 1;
        }
        else if (tmp_char == tmp[i]){
            ++tmp_len;
        }
        else if (tmp_char != tmp[i]){
            if (tmp_len > cont_len){
                ...
                sprintf(result, "%c", tmp_char); // with format
            }
            else if (tmp_len == cont_len){
                sprintf(result, "%s %c %d", result, tmp_char, tmp_len); // with format
    ...
    if (cont_len)
        sprintf(result, "%s %d", result, cont_len); // with format
    else 
        sprintf(result, "0");
    puts(result);
}
...
int main(void){
    ...
    while (fgets(cmd, 0x7fffffff, stdin)){
        ...
        else if (!strcmp(cmdPtr, "call_out\n")){ // debug
            char tmp[BLOCK_LEN] = "";
            print(head, tmp);
            puts(tmp);
            continue;
